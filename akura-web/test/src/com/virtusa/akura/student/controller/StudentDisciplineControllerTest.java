...
public class StudentDisciplineControllerTest extends BaseWebTest {
    ...
    /** Holds StudentDisciplineController instance. */
    private StudentDisciplineController studentDisciplineController;
    ...
    /** Holds StudentDiscipline instance. */
    ...
    public final void setUp() throws AkuraAppException {
        ...
        warningLevel.setDescription("Warning Level");
        warningLevel.setColor("Color");
        warningLevel.setModifiedTime(new Date());

        warningLevel = commonService.addWarningLevelsInfo(warningLevel);
        assertNotNull(" Warning Level saved to db " + student);

        studentDiscipline = new StudentDiscipline();
        studentDiscipline.setComment("NewComment");
        studentDiscipline.setStudentId(student.getStudentId());
        studentDiscipline.setWarningLevelId(warningLevel.getWarningLevelId());
        studentDiscipline.setInformedtoParent(false);
        studentDiscipline.setDate(new Date());
        studentDiscipline.setModifiedTime(new Date());

        studentDisciplineController = new StudentDisciplineController();
        studentDisciplineController.setStudentService(studentService);
        ...
        model.addAttribute("studentDiscipline", studentDiscipline);
    ...
    public void testSaveOrUpdateStudentDiscipline() throws AkuraAppException {
        ...
        try {
            ...
            studentDisciplineController.setStudentDisciplineValidator(studentDisciplineValidator);
            Mockito.when(mockBindingResult.hasErrors()).thenReturn(true);
            String view =
                    studentDisciplineController.saveOrUpdateStudentDiscipline(studentDiscipline, mockBindingResult,
                            request, model, session);

            assertEquals("Student/studentDiscipline", view);

            Mockito.when(mockBindingResult.hasErrors()).thenReturn(false);
            String savedView =
                    studentDisciplineController.saveOrUpdateStudentDiscipline(studentDiscipline, mockBindingResult,
                            request, model, session);
            assertEquals("redirect:studentDiscipline.htm", savedView);
            assertTrue(model.size() > 0);

            request.addParameter("studentDisciplineId", "" + studentDiscipline.getStudentDisciplineId());
    ...
    public void tearDown() throws AkuraAppException {
        ...
        studentDisciplineController.deleteStudentDiscipline(request, model);
        ...
        commonService.deleteWarningLevelsInfo(warningLevel);
    }
